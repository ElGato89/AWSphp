  ...
  <header>
    <!-- header inner -->
     ...
     <!-- end header -->
        ...
        ?>
  <h1>Insert New Concert Details</h1>
  <?php
  if (!empty($_POST)){
      $name = strtoupper(trim($_POST["txtConcertName"]));
      $date = trim($_POST["txtPrice"]);
      $time = trim($_POST["txtTime"]);
      $venue = trim($_POST["txtVenue"]);
      $price = trim($_POST["txtPrice"]);
      
      $con = new mysqli (DB_HOST, DB_USER, DB_PASS, DB_NAME);
      $sql = "INSERT INTO upcoming_concert (concert_name, concert_date, concert_time, concert_venue, concert_price) VALUES (?,?,?,?,?)";
      $stmt = $con->prepare($sql);
      $stmt->bind_param("ssssd", $name, $date, $time, $venue, $price);
      $stmt->execute();
      
      if($stmt->affected_rows > 0){
          printf("<div>Concert <b>%s</b> has been added. [<a href='concert-details-admin.php'>Back to Concert Details]</div>", $name);
      }else{

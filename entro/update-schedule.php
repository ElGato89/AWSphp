    ...
    <!-- header inner -->
     ...
     <!-- end header -->
        ...
        if($_SERVER["REQUEST_METHOD"]=="GET"){
            ...
            $sql = "SELECT * FROM schedule WHERE concert_name = '$name'";
            ...
            if($row = $result->fetch_object()){
               ...
               echo "Unable to process.[<a href='schedule-admin.php'>Try Again</a>]"; 
            }
            $result->free();
            $con->close();
        }else{
            ...
            $con = new mysqli (DB_HOST, DB_USER, DB_PASS, DB_NAME);
            $sql = "UPDATE schedule SET concert_name = ?, concert_price = ? WHERE concert_name = ? ";   
            $stmt = $con->prepare($sql);
            //note: s - string, d - double, i - integer, b - blob
            $stmt->bind_param("sds", $name, $price, $name); 
            //execute the prepared statement
            $stmt->execute();           
            
            if($stmt-> affected_rows > 0){ 
                printf("<div>Schedule <b>%s</b> has been updated...[<a href='schedule-admin.php'>Back to Schedule</a>]</div>",$name);
            }else{
        ...
        <form method="POST" action="">
            <table>
                ...
                <td>Concert Name:</td>
                <td><?php echo $name;?>
                <input type="hidden" name="txtConcertName" value="<?php echo $name; ?>"/>
                </td> <!-- the concert name is static, only can change price in the edit function -->

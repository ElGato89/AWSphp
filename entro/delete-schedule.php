...
<script>
        function toCancel() {
            window.location.href = 'schedule-admin.php';
        }
    </script>  
        <?php
        ...
        if($_SERVER["REQUEST_METHOD"]=="GET"){
            ...
            if($row = $result->fetch_object()){
                ...
                printf("<p>Are you sure you want to delete the following record?</p>
                        ...
                        <input type='button' value='Cancel' name='btnCancel' onclick='toCancel()'/>
                        </form>
                        </table> ", $name, $price, $name, $price);
            ...
            $con = new mysqli (DB_HOST, DB_USER, DB_PASS, DB_NAME);
            $sql = "DELETE FROM schedule WHERE concert_name = ?";
            ...
            if($stmt->affected_rows> 0){
                printf("<div>
                        Schedule <b>%s</b> has been deleted!
                        [<a href='schedule-admin.php'>Back to schedule</a>]
                        </div>",$name);
            }else{
                echo "<div>
